...
long previousMillis = 0; //Время последнего нажатия кнопки - переменная для борьбы с дребезгом
...
boolean beep_stopped = false;  // Флаг "писк остановлен". иначе не получается прервать писк без последствий )(

void setup(){
attachInterrupt(0, _pressButton, LOW); // включаем режим обработки прерывания. Допустимо использование следующих констанст:
...
void beep(int vDelay){
  for (int i=0;i<=1000;i++){ // собственно "колебаем" пьезо 1000 раз
    ...
    if( beep_stopped == true) break;   // принудительно завершим генерацию писка. Необходимо если срабатывает прерывание, которое меняет статутс запущено/остановлено).
                                       // ... т.к. после обработки прерывание возвращается в передыдущее состояние, а писк-то нам уже не нужен )))
  }
  beep_stopped = false; // включим "умение пищать", включение в loop иногда не хватает при очень быстром нажатии на кнопку
}
...
beep_stopped = false; // включим "умение пищать", оно может быть погашено прерыванием
...
void _pressButton() {
  if (millis() - previousMillis > 100) { // условие для борьбы с дребезгом (если кнопку нажали быстро очень, то второй раз не сработает.
    timer_stopped = !timer_stopped;   // инвертируем флаг
    previousMillis = millis();        // схороним время нажатия кнопки (как без этого не глючило - видимо свезло)
    beep(200);                        // Подтверждающий бип, он же небольшая сглаживающая задержка
    beep_stopped = true;              // остановим писк, если пищит.
  }
}
