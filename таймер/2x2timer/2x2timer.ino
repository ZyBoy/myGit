...
void setup(){
  TIMSK2=1; // Признак многозадачности, без него работать не будет
...
void loop1() {
  if(digitalRead(BUTTON)==LOW) // Проверяем нажатие кнопки, что бы можно было остановить/переключить таймер в любом состоянии
  {
    timer_stopped = !timer_stopped;   // инвертируем флаг на "останов"
