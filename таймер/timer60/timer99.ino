...
if (timer_stopped == false){       // Если таймер не остановлен, то запускаем циклы счетчика...
  for (int ten=0;ten<10;ten++){     // ... сперва десятки секунд до 9 включительно
    for (int one=0;one<10;one++){  // ... затем единицы секунд до 9 включительно
      ...
      //  - если отображаются последние 3 секунды таймера текущего и превышеного
        else if ((ten==5 && (one == 9 || one == 8 || one == 7)) || (ten==9 && (one == 8 || one == 7 || one == 6))){
          beep(200);
