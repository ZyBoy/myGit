...
if (timer_stopped == false){       // Если таймер не остановлен, то запускаем циклы счетчика...
  for (int ten=0;ten<6;ten++){     // ... сперва десятки секунд
    for (int one=0;one<10;one++){  // ... затем единицы секунд
      set_digit(5-ten, true);      // рисуем цифру слева
      set_digit(9-one, false);     // ... и справа
      
      // Дополнительное условие для звукового сигнала
      //  - если отображается 30 секунд
      //  - если отображается 3, 2 и 1 секунды
      if ((ten==2 && one == 9) || (ten==5 && (one == 6 || one == 8 || one == 7))){
        beep(200);
      ...
      // Условие на завершения таймера. Звук другой, поправка уже не нужна, дальше не тикает.
      if (ten==5 && one == 9) beep(400);
      ...
      for (int timer_pause=0;timer_pause<(1000-popravka);timer_pause++){
        if(digitalRead(BUTTON)==LOW) // Заодно проверяем нажатие кнопки, что бы можно было остановить таймер в любом состоянии
        {
          timer_stopped = true;   // переключаем флаг на "останов"
          beep(200);              // Подтверждающий бип
          return;                 // прерываем текущее выполнение функции loop, и все начинается с начала )))
        }
